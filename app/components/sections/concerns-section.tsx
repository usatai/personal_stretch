...
const ConcernsSection = () => (
  <section id="concerns" className="on-dark relative bg-slate-950 overflow-hidden">
    {/* 装飾グロー */}
    ...
    <div className="relative max-w-5xl mx-auto px-6 sm:px-10 lg:px-12 py-24 sm:py-32">
      ...
      <ScrollReveal className="mb-16 md:mb-20">
        <p className="flex items-center gap-3 text-cyan-400 text-xs font-black tracking-[0.28em] uppercase mb-5">
        ...
        {concerns.map((c, i) => (
          <ScrollReveal key={i} delay={i * 65} direction="left">
            <div className="group relative flex items-center gap-5 md:gap-10 py-8 md:py-10 border-b border-white/8 cursor-default overflow-hidden">
              ...
              {/* 番号 */}
              <span className="relative shrink-0 w-8 text-right font-mono text-xs font-black text-cyan-500 group-hover:text-cyan-400 tracking-widest transition-colors duration-300">
                {c.num}
              </span>
              ...
              <div className="relative flex-1 min-w-0">
                ...
                <p className="mt-1.5 text-xs font-bold tracking-[0.25em] uppercase text-white/50 group-hover:text-cyan-400 transition-colors duration-300">
                  {c.en}
                </p>
              </div>
            </div>
      ...
      <ScrollReveal className="mt-14 flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <p className="text-white/60 text-base font-medium">ひとつでも当てはまる方へ —</p>
        <Link
          href="/contact"
          className="text-cyan-400 hover:text-cyan-300 font-bold text-base flex items-center gap-1.5 transition-colors duration-200"
        >
